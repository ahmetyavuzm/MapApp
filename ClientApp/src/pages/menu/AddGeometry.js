...
const AddGeometry = () => {
  ...
  React.useEffect(() => {
    ...
    if (editItem) {
      ...
      console.log("Info: ", editItem.info);
      if (editItem.info && editItem.info.groupId) {
        console.log("Parents: ", getParents(editItem.info.groupId));
        setParents(getParents(editItem.info.groupId));
      }
      if (editItem.info && editItem.info.type) {
        if(editItem.info.type.toLowerCase() === "layer"){
          setParents([]);
        }
        setType(editItem.info.type);
      }
    }
  }, [editItem]);
  ...
  React.useEffect(() => {
    if (parents) {
      ...
      let groups = last_parent.geometries.filter((geometry, index) => {
        return geometry.type.toLowerCase() === "group";
      });
      if (groups.length > 0) {
  ...
  const handleSubmit = async () => {
    ...
    if (type.toLowerCase() === "group" || type.toLowerCase() === "layer") {
        let data = {
          type: type,
          properties: [
          ...
          groupId: type.toLowerCase() === "layer" ? null :
      ...
      let res = await addGeometry(data);
      console.log(res);
    }
  };
  ...
  return (
    <div className="w-full h-full p-3 overflow-y-auto">
      <div className="pb-3">
        <label className="text-3xl m-1">Add Item</label>
          ...
          return (
            <div className=" " key={index}>
              ...
              >
                ...
                {groups.map((group, i) => {
                  const group_name = group.properties.filter((prop, index) => {
